...
import com.zzjj.consistency.controller.data.RegisterOrCancelRequest;
...
/**
 * 公用的请求对象 当我们的框架伴随着你的系统启动之后，系统一定会扫描你的框架的所有bean 在这里会扫描到你对应的controller bean
 * 把他变成可以对外处理http请求的bean，这个时候我们的框架controller也可以去处理你外部的请求了 框架之间的online请求，/common/registerOrCancel
 * 
 * @author zengjin
 * @date 2023/11/19
 **/
...
public class CommonController {
    ...
    public CommonRes<?> leaderHeartbeat(@RequestBody RegisterOrCancelRequest registerOrCancelRequest) {
        ...
        if (isLeader) {
            registerResponse = RegisterOrCancelResponse.builder().leader(true).replyPeerId(currentPeerId)
                .registerOrCancelRequest(registerOrCancelRequest).build();
        } else {
            registerResponse = RegisterOrCancelResponse.builder().leader(false).replyPeerId(currentPeerId)
                .registerOrCancelRequest(registerOrCancelRequest).build();
        }

        // 你作为leader，你自己内部，会去发布一个事件，当前就是有一个RegisterOrCancelResponse事件发生了
        // 有一个新的节点online上线了
        this.peerElectionHandler.eventBus.post(registerResponse);
